...
可在 Temporal Workflow 內直接呼叫而不破壞決定性。

正式版落在 python-ai/policy/engine.py，介面不變。
"""
...
def evaluate(
    ...
    if satisfied:
        return _apply_result_policy(
            policy.result, approved=approved, rejected=rejected, total=total_participants
        )
