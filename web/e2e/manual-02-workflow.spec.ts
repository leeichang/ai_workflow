...
test.describe('操作手冊：從零建立流程', () => {
  ...
  test('登入到流程設計完成', async ({ page }) => {
    ...
    // 業務物件必須有 schemas/business-objects/*.json 定義，
    // 條件式才引用得到路徑。沒有定義的話會被 WF-E012 擋下。
    await page.getByTestId('new-business-object').fill('leave_request')
